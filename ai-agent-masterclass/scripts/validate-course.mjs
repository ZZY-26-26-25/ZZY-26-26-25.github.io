...
check(data.meta.phase === "theory-foundation", "meta.phase 应标明 theory-foundation");
...
for (const module of data.modules || []) {
  ...
  check(module.status === "theory", `模块 ${module.id} 未标为 theory`);
...
check(
  !/护照[^。；]{0,120}到期月[^。；]{0,120}(?:满足|有效|通过)/.test(publicText),
